...
A **holding page**: the WHY8 Foundation lockup, the name, the one-liner, the purpose paragraph ("Our Purpose") and `info@why8.org`. Both texts are the register claim `why8.purpose.charter` (esposure-ops migration 256, 2026-09-14), cut verbatim from the founders' purpose clause. Nothing else, on purpose — programs, partners, numbers and legal status arrive as claims with the entity and its determination letter. Until then the site must not print `501(c)(3)`, "tax-deductible"/"tax-exempt", or "is a Texas nonprofit corporation" (it is "being organized as" until the certificate of formation is filed); `npm run check:claims` fails locally if any of those appear in the pages.
